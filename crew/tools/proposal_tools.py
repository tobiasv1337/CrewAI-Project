...
class ProposalCourseInput(BaseModel):
    ...
    isis_course_id: int | None = Field(default=None, description="Verified ISIS course id for permanent enrollment.")
    ...
    *,
    ...
    for course in courses:
        ...
        if course.include_grade_manager:
            payload = {
                "module_query": course.module_query or course.course_title,
        ...
        if course.include_isis:
            payload = {
                "course_id": course.isis_course_id,
                "course_query": course.isis_course_query,
                "course_url": course.isis_course_url,
                "term_hint": course.isis_term_hint or course.term,
                "expected_title": course.course_title,
            }
            actions.append(
                ProposedAction(
                    action_id=_stable_action_id("isis_enroll", course.course_title, payload),
                    kind="isis_enroll",
                    course_title=course.course_title,
                    evidence=[course.rationale, *course.evidence],
