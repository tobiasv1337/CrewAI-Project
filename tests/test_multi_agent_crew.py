...
from crew.tools import MOSES_MODULE_RESEARCH_TOOLS, STUDY_ADVISOR_TOOLS
...
def test_multi_agent_crew_uses_hierarchical_manager_and_specialist_tools(monkeypatch):
    ...
    assert len(built_crew.agents) == 3
    ...
    assert "Add Module To Study Plan" in [tool.name for tool in study_advisor.tools]
